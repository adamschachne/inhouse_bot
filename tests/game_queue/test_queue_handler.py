...
async def test_queue_full():
    ...
    await game_queue.add_player(0, roles_list[0], 1, 0, name="0")
    assert len(GameQueue(1)) == 1

    # We queue our player 0 for every other role in channel 0
    for role in roles_list:
        await game_queue.add_player(0, role, 0, 0, name="0")
    ...
    assert (
        len(GameQueue(0)) == 0
    )  # Player 0 should not be counted in queue for any role anymore
    ...
    with pytest.raises(game_queue.PlayerInReadyCheck):
        await game_queue.add_player(0, roles_list[0], 2, 0, name="0")
    ...
    await game_queue.add_player(10, roles_list[0], 0, 0, name="10")
...
async def test_queue_remove():
    ...
    await game_queue.add_player(0, roles_list[0], 0, 0, name="0")
...
async def test_duo_queue():
    ...
    await game_queue.add_duo(
        0, "TOP", 9, "SUP", 0, 0, first_player_name="0", second_player_name="9"
    )
